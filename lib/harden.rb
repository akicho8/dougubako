...
module Harden
  class Core
    ...
    def src_files_collect
      target_dirs.each do |target_dir|
        Pathname.glob("#{target_dir}/**/*") do |filename|
          ...
          if @params[:filemask]
            unless filename.to_s.match(@params[:filemask])
              @filter_files << filename
              next
            end
          end
    ...
    def header
      ...
      o.puts "* ファイルリスト"
      o.puts ""
      @src_files.each_with_index do |filename, i|
        o.puts sub_header(i, filename)
      end
      o.puts ""
      o.puts "* ディレクトリリスト"
      o.puts ""
      @src_files.collect{|e|e.dirname}.uniq.each_with_index do |e, i|
      ...
      if @heavy_files.size >= 1
        o.puts ""
        o.puts "* サイズが大きすぎてフィルタしたファイル"
        o.puts ""
        @heavy_files.each_with_index do |e, i|
          o.puts "%4d %s" % [i.succ, e]
        end
      end
      o.string
    ...
    def body
      ...
      @src_files.each_with_index do |filename, i|
        ...
        o.puts "* #{sub_header(i, filename)}"
        o.puts "-" * 80
        NKF::nkf("--unix", filename.read).lines.each do |line|
  ...
  module CLI
    def self.execute(args)
      ...
      oparser = OptionParser.new do |opts|
        ...
        opts.on("--help", "このヘルプを表示する") {puts opts; exit}
        opts.on(<<-EOT)
