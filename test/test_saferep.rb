...
class TestReplace < Test::Unit::TestCase
  ...
  def test_help
    # assert_equal(1, `../bin/saferep`.lines.grep(/saferep --help/).size)
    # assert_equal(1, `../bin/saferep --help`.lines.grep(/--exec/).size)
  end

  def test_main
    assert_equal(1, `../bin/saferep    foo bar testdir`.lines.grep(/→【bar _bar_ FOO】/).size)
    assert_equal(1, `../bin/saferep -w foo bar testdir`.lines.grep(/→【bar _foo_ FOO】/).size)
    assert_equal(1, `../bin/saferep -i foo bar testdir`.lines.grep(/→【bar _bar_ bar】/).size)
  end

  def test_option_x
    assert_equal(1, `../bin/saferep -x foo bar testdir`.lines.grep(/→【bar _bar_ FOO】/).size)
  ...
  def test_block_replace
    assert_equal(1, `../bin/saferep '_(\\w+)_' '@\#{\$1}@' testdir`.lines.grep(/→【foo @foo@ FOO】/).size)
  end
